...
def extract_text_from_docx(file_path):
    ...
    return text, total_cells, empty_cells, filled_cells
...
pt_text, pt_total_cells, pt_empty_cells, pt_filled_cells = extract_text_from_docx(pt_file)
en_text, en_total_cells, en_empty_cells, en_filled_cells = extract_text_from_docx(en_file)
...
with open(diff_report_path, "w", encoding="utf-8") as report:
    ...
    report.write(f"Total de células na tabela: {pt_total_cells}\n")
    report.write(f"Células preenchidas: {pt_filled_cells}\n")
    report.write(f"Células vazias: {pt_empty_cells}\n\n")

    report.write(f"Arquivo em Inglês: {en_file}\n")
    report.write(f"Palavras lidas: {en_word_count}\n")
    report.write(f"Total de células na tabela: {en_total_cells}\n")
    report.write(f"Células preenchidas: {en_filled_cells}\n")
    report.write(f"Células vazias: {en_empty_cells}\n\n")
    ...
    if differences:
        ...
        for i, (pt, en, translated, similarity) in enumerate(differences, start=1):
            report.write(f"Diferença {i}:\n")
